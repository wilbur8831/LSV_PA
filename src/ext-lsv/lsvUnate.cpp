...
static bool isPositiveUnate(sat_solver* pSat, Cnf_Dat_t* pCnfPos,
                            ...
                            Aig_Man_t* pMan, int pIndexCo, int pIndexCi) {
  return isUnate(pSat, pCnfPos, pCnfNeg, pVecEnVars, pMan, pIndexCo, pIndexCi,
                 true);
}
  ...
  Aig_ManForEachCo(pMan, pCo, i) {
    Aig_ManForEachCi(pMan, pCi, j) {
      ...
      if (isPositiveUnate(pSatSolver, pCnfPositiveCofactor,
                          pCnfNegativeCofactor, vEnableVars, pMan, i, j)) {
        isPosUnate = true;
...
bool isUnate(sat_solver* pSat, Cnf_Dat_t* pCnfPos, Cnf_Dat_t* pCnfNeg,
             Vec_Int_t* pVecEnVars, Aig_Man_t* pMan, int pIndexCo, int pIndexCi,
             bool pPhase) {
  ...
  assumptions[Aig_ManCiNum(pMan) + 2] = toLitCond(
      pCnfPos->pVarNums[Aig_ObjId(Aig_ManCo(pMan, pIndexCo))], pPhase);
  assumptions[Aig_ManCiNum(pMan) + 3] = toLitCond(
      pCnfNeg->pVarNums[Aig_ObjId(Aig_ManCo(pMan, pIndexCo))], !pPhase);
